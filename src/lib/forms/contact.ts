...
export function createContactValidationResult(error: ZodError) {
  return buildContactResult({
    fieldErrors: getContactFieldErrors(error),
...
export async function submitContactForm(
  ...
  try {
    ...
    if (!verification.success) {
      return buildContactResult({
        fieldErrors: {},
        message: "BOT 対策の確認に失敗しました。もう一度お試しください。",
        ok: false,
        resetTurnstile: true,
        status:
      ...
      return buildContactResult({
        ...
        message:
          webhookResult.reason === "missing-discord-webhook"
            ...
            : "Discord への送信に失敗しました。時間をおいて再度お試しください。",
        ok: false,
        resetTurnstile: true,
        status:
          webhookResult.reason === "missing-discord-webhook"
    ...
    return buildContactResult({
      fieldErrors: {},
      message: "送信が完了しました。Discord に通知しています。",
      ok: true,
      resetTurnstile: true,
      status: "success",
      turnstileErrors: [],
