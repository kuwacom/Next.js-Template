...
export function ApiContactForm({
  locale,
  pagePath,
  siteKey,
}: ApiContactFormProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // API Route へ送る前に client 側でも同じ schema を通し、表示エラーを即時に返す
  ...
  return (
    <Card className="border-border/60 shadow-sm">
      <CardHeader>
        <CardTitle>{t("cardApiTitle")}</CardTitle>
        <CardDescription>{t("cardApiDescription")}</CardDescription>
      </CardHeader>
      <CardContent>
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
          <div className="grid gap-5 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="api-name">{t("nameLabel")}</Label>
              <Input
                id="api-name"
                name="name"
            ...
            <div className="space-y-2">
              <Label htmlFor="api-email">{t("emailLabel")}</Label>
              <Input
                id="api-email"
                name="email"
                type="email"
          ...
          <div className="space-y-2">
            <Label htmlFor="api-company">{t("companyLabel")}</Label>
            <Input
              id="api-company"
              name="company"
          ...
          <div className="space-y-2">
            <Label htmlFor="api-message">{t("messageLabel")}</Label>
            <Textarea
              id="api-message"
              name="message"
          ...
          >
            {isMutating ? t("submitApiPending") : t("submitApi")}
          </Button>
        </form>
      </CardContent>
    </Card>
